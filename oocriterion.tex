...
\begin{document}
\chapter{Use of Failure Criteria for AM} \label{ch:oocrit}
...
\begin{enumerate}
	...
	\end{equation}
	 where $\frac{d\tau_{12}}{d\sigma_{22}}$  is the slope of the graph at failure under shear. This term is named $\mu^{2212}$ in the SSIC and can be obtained by performing combined loading tests. Refer to Figure \ref{fig:OOCdemo} for a visual representation.
...
Applying the SSIC to the data shown in Figure \ref{fig:GKClimit} demonstrates how the failure surface developed through this new criterion better reflects the failure behavior than its GKC counterpart. A comparison between the two can be seen in Figure \ref{fig:GKOOComp}. Note how the SSIC envelope captures the shear strengthening better than the GKC. 
\begin{figure}[h]
	\center
	\includegraphics[height=7cm]{GK_OO}
	\caption{Comparison of GKC and SSIC failure envelopes \cite{Osswald2017a}} \label{fig:GKOOComp}
\end{figure}
...
This work will apply the SSICC to FFF, in the hopes that it becomes a tool for safely designing parts intended to be manufactured through this process. Chapter \ref{ch:exp} will detail the material and experimental methods used to achieve this goal.
